...
{
    int count = 0, total_count=0, count_in;
    ...
    for (int i = 1; i < log; i++)
    {
        if (runner == 1){
            if ((1 <<  i) & rank) {
                MPI_Send(&count, 1, MPI_REAL, rank-((int) pow(2,i)), 0, MPI_COMM_WORLD); 
                runner = 0;
            }
            else{
                MPI_Recv(&count_in, 1, MPI_REAL, rank+((int) pow(2,i)), 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
                count += count_in;
            }
        }
